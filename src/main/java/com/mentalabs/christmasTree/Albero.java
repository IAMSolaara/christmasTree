...
public class Albero {
    ...
    /** The tree itself in int matrix form. 
     * Uses the above mentioned elements 
     */
    private int[][] pic;
        ...
        for (int i = 1; i < height; i++) {
            ...
            for (int j = 0; j < width; j++) {
                ...
                // if inside check if current cell isnt a branch
                    ...
                    //check if next integer is a multiple of 4, used to get more random chances and if yes then place a random kind of element
                    if (rng.nextInt(9) % 4 == 1)
                        pic[i][j] = rng.nextInt(3) + PALLINA_0;
                    //else empty the area, probably not needed and reduntant, doesnt harm nobody though
                    else
                        pic[i][j] = SFONDO;
                }
            }
    ...
    public String toString() {
        ...
        for (int i = 0; i < height; i++) {
            //cycle through line
            for (int j = 0; j < width; j++) {
                //see if current cell is a certain element and concat a symbol corresponding to said element
                switch (pic[i][j]) {
                case RAMO_0:
                    out += "\033[32m=";
                    break;
                ...
                case LUCINA_SPENTA:
                    ...
                    break;
                case LUCINA_ACCESA:
                    //give some random color to the light for added flare
